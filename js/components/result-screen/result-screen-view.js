...
const successTemplate = (gameState) => {
  const time = gameData.START_TIME - gameState.time;
  const fastAnswers = gameState.answers.filter((answer) => {
  ...
  return `
    ...
    <div class="main-stat">За&nbsp;${(time - time % 60) / 60}&nbsp;минуты и ${time % 60}&nbsp;секунд
      <br />вы&nbsp;набрали ${countScore(gameState.answers)} баллов (${fastAnswers} быстрых)
      <br />совершив ${gameState.mistakes} ошибки</div>
